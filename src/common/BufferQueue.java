...
/***
 * 
 * @author Spencer Ruport (http://www.siafoo.net/snippet/216).
 *
 */
...
public class BufferQueue {
	...
	public void append(byte[] data, int offset, int length) {
		if(data == null) return;
		if(data.length < offset + length) { throw new RuntimeException("array index out of bounds. offset + length extends beyond the length of the array."); }

		try { appendsem.acquire(); } catch (InterruptedException e) { return; }
		...
		try { countsem.acquire(); } catch (InterruptedException e) { return; }
	...
	public int read(byte[] data, int offset, int length) {
		...
		try { countsem.acquire(); } catch (InterruptedException e) { readsem.release(); return 0; }
		// We need to acquire the semaphore so that this.count doesn't change.
	...
	public byte[] readBytes() 
	{
		...
		try { read(data); } catch(Exception ex) {}
		return data;
	}
}
