...
import common.map.TreeFactory;
import common.map.TreeFactory.Tree;
...
public class TreeTest {
	...
	public static void main(String[] args) throws Exception {
		...
		TreeFactory tf = new TreeFactory((long) 32);
		//adding random trees(
		for(int i =0 ; i<100; i++){
			Tree tree = (Tree)tf.getTree(Vec3.create(Math.random()*SIZE*Segment.horzScale, 2, Math.random()*SIZE*Segment.horzScale));
			scene.addDrawables(tree.getMeshContexts());
		}
